  ...
  await db.transaction(async (tx) => {
    ...
    await tx
      ...
      .values(
        orderSeeds.map((order) => ({
          ...
          initialEstimatedDeliveryDateEnd: dateOnly(addDays(now, order.dueOffsetDays + 5)),
          currentEstimatedDeliveryDate: dateOnly(addDays(now, order.dueOffsetDays)),
          currentEstimatedDeliveryDateEnd: dateOnly(addDays(now, order.dueOffsetDays + 5)),
    ...
    await tx
      ...
      .values({
        ...
        previousDateEnd: dateOnly(addDays(now, 8)),
        newDate: dateOnly(subDays(now, 2)),
        newDateEnd: dateOnly(addDays(now, 3)),
